...
- The selected group, subgroup, or server must use a single consistent sidebar selection highlight; selecting a new item must clear the previous highlight. Starting navigation from the server filter must transfer focus to the list, GTK expander focus must not create a second selector, and `Up` must never leave the list for the sidebar action buttons while an earlier visible row exists. If GTK temporarily focuses a sidebar action button during active list navigation, the next navigation event must remain in the list; focusing a terminal must disable this sidebar navigation mode.
