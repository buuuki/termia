...
class TerminalSessionsMixin:
    ...
    def on_send_files_to_server(self, popover: Gtk.Popover, server: Server) -> None:
        ...
        dialog.open_multiple(
            self,
            None,
            lambda current_dialog, result: self.on_send_files_to_server_selected(current_dialog, result, server),
        )

    def on_send_files_to_server_selected(
        self,
        ...
        if not local_paths:
            return
        self.start_scp_upload(server, local_paths)

    def start_scp_upload(self, server: Server, local_paths: list[Path]) -> None:
        ssh_path = GLib.find_program_in_path("ssh")
        ...
        dialog_state = self.show_file_transfer_dialog(server, destination, file_list)
        dialog_state["status"].set_label(self.t("send_files_to_server_prepare_remote"))
        self.run_file_transfer_command(
            ssh_command,
            server.password if use_sshpass else "",
            dialog_state,
            lambda: self.start_scp_copy_step(server, scp_command, use_sshpass, dialog_state),
        )
        self.toast_label.set_label(self.t("send_files_to_server_started").format(name=server.name))

    def show_file_transfer_dialog(self, server: Server, destination: str, file_list: str) -> dict[str, Any]:
        dialog = Gtk.Dialog(title=self.t("send_files_to_server_title").format(name=server.name), transient_for=self, modal=False)
        dialog.set_resizable(False)
        ...
        state["pulse_id"] = GLib.timeout_add(120, self.pulse_file_transfer_progress, state)
        dialog.connect("response", self.on_file_transfer_dialog_response, state)
        dialog.present()
        return state

    def pulse_file_transfer_progress(self, state: dict[str, Any]) -> bool:
        progress = state.get("progress")
        if isinstance(progress, Gtk.ProgressBar):
            progress.pulse()
        return GLib.SOURCE_CONTINUE

    def on_file_transfer_dialog_response(
        self,
        dialog: Gtk.Dialog,
        ...
        if state.get("completed"):
            self.cleanup_file_transfer_dialog(state)
            dialog.destroy()
            return
        if response == Gtk.ResponseType.CANCEL:
            ...
            if isinstance(process, Gio.Subprocess):
                process.force_exit()
            self.finish_file_transfer_dialog(state, self.t("send_files_to_server_cancelled"), failed=True)
            return
        self.cleanup_file_transfer_dialog(state)
        dialog.destroy()

    def start_scp_copy_step(
        self,
        server: Server,
        ...
        self.run_file_transfer_command(
            ...
            server.password if use_sshpass else "",
            state,
            lambda: self.finish_file_transfer_dialog(
                state,
                self.t("send_files_to_server_finished").format(name=server.name),
            ),
        )

    def run_file_transfer_command(
        self,
        command: list[str],
        password: str,
        state: dict[str, Any],
        ...
        try:
            ...
            self.finish_file_transfer_dialog(
                state,
        ...
        process.communicate_utf8_async(
            None,
            state["cancellable"],
            lambda current_process, result: self.on_file_transfer_command_finished(
                current_process,
                result,
                state,
                on_success,
    ...
    def on_file_transfer_command_finished(
        self,
        ...
        try:
            ...
            if state["cancellable"].is_cancelled():
                return
            self.finish_file_transfer_dialog(state, self.t("send_files_to_server_failed_generic"), failed=True)
            return
        if not process.get_successful():
            detail = self.last_transfer_output_line(stdout, stderr)
            message = self.t("send_files_to_server_failed_detail").format(error=detail)
            self.finish_file_transfer_dialog(state, message, failed=True)
            return
        state["process"] = None
        on_success()

    def last_transfer_output_line(self, stdout: str | None, stderr: str | None) -> str:
    ...
    def finish_file_transfer_dialog(self, state: dict[str, Any], message: str, failed: bool = False) -> None:
        self.cleanup_file_transfer_dialog(state)
    ...
    def cleanup_file_transfer_dialog(self, state: dict[str, Any]) -> None:
